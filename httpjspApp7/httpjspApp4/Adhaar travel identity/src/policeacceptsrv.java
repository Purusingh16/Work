...
public class policeacceptsrv extends HttpServlet 
{
	Connection con;
	...
	catch(Exception ee)
	{
      ...
      public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		String email=req.getParameter("policeaccept");
		
		PreparedStatement ps=con.prepareStatement("select * from userpassport where emailid=?");
		...
		String policestationname=req.getParameter("t1");
		String policeinchargename=req.getParameter("t2");
		PreparedStatement ps1=con.prepareStatement("insert into passporttablepermanent values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	    ...
	    pw.println(" DATAS ARE PERMANENTLY ACCEPTED<br/>");
